...
def base_row(index: int, label: str) -> dict:
    ...
    action = random.choice(["start-N1", "start-N2", "set-fuel", "restore-cooling", "restore-ventilation", "protect-K1", "protect-K2", "toggle-electric-field"])
    expected = action
    return {
        ...
        "action": action,
        "previousAction": random.choice(["open-L1", "start-N1", "restore-cooling", "check-alarm", "set-fuel-45"]),
...
def mutate_action(row: dict) -> dict:
    ...
    if label == "SEQUENCE_ERROR":
        row.update(action="start-N1", previousAction=random.choice(["set-fuel-70", "check-alarm", "drain-E1"]), expectedAction="open-L1", valveL1=round(random.uniform(0, 88), 1), feedFlow=round(random.uniform(0, 280), 1))
    elif label == "RESPONSE_TIME_ERROR":
        row.update(responseSeconds=round(random.uniform(61, 150), 2), pauseSeconds=round(random.uniform(61, 150), 2), previousAction="alarm-K1-high", expectedAction=random.choice(["restore-cooling", "protect-K1", "cut-fuel"]))
        if b(.5):
        ...
        repeated = random.choice(["set-fuel-45", "restore-cooling", "start-N1", "toggle-electric-field"])
        row.update(action=repeated, previousAction=repeated, expectedAction="check-process-state", duplicateCount=random.randint(2, 5), pauseSeconds=round(random.uniform(.2, 3), 2))
    elif label == "PARAMETER_ERROR":
        row.update(action="set-fuel", actionValue=round(random.uniform(70, 100), 1), expectedAction="set-fuel-45", feedFlow=round(random.uniform(100, 420), 1), temperatureFurnace=round(random.uniform(366, 430), 1))
        if b(.45):
            row["pressureK1"] = round(random.uniform(4.0, 4.8), 2)
    elif label == "EQUIPMENT_NOT_READY":
        variant = random.choice(["power", "air", "valve", "cooling"])
        row.update(action=random.choice(["start-N1", "start-N2", "set-fuel"]), expectedAction="restore-utility-or-prepare-equipment")
        if variant == "power": row["powerOk"] = False
        elif variant == "air": row["instrumentAirOk"] = False
        elif variant == "valve": row["valveL1"] = round(random.uniform(0, 85), 1)
        else: row.update(coolingWaterOk=False, pressureK1=round(random.uniform(3.8, 4.5), 2))
    elif label == "CRITICAL_OPERATION_ERROR":
        variant = random.choice(["furnace", "k1", "k2", "gas"])
        if variant == "furnace": row.update(action="set-fuel", actionValue=round(random.uniform(75, 100), 1), feedFlow=round(random.uniform(0, 180), 1), temperatureFurnace=round(random.uniform(390, 455), 1), expectedAction="restore-circulation-and-reduce-fuel")
        elif variant == "k1": row.update(action="set-fuel", actionValue=round(random.uniform(50, 90), 1), levelK1=round(random.uniform(2, 14), 1), expectedAction="protect-K1")
        elif variant == "k2": row.update(action="set-fuel", actionValue=round(random.uniform(50, 90), 1), levelK2=round(random.uniform(2, 14), 1), expectedAction="protect-K2")
        else: row.update(action="disable-ventilation", gasPercent=round(random.uniform(15, 35), 1), ventElouOk=False, expectedAction="restore-ventilation")
    return row
...
def build_risk(actions: list[dict]) -> list[dict]:
    ...
    for index, source in enumerate(actions, 1):
        ...
        elif actual_label in {"SEQUENCE_ERROR", "RESPONSE_TIME_ERROR", "PARAMETER_ERROR", "EQUIPMENT_NOT_READY"}:
        ...
        row.update({
            ...
            "riskClass": "HIGH" if risk_score >= 70 else "MEDIUM" if risk_score >= 35 else "LOW",
