"""Каталог мини-тренингов + evaluate (JSON, без БД)."""
...
def evaluate(training_id: str, process: dict, hints_used: int) -> dict:
    ...
    criteria = training.get("criteria") or []
    if not criteria:
        return {
    ...
    checks = [check_condition(condition, process) for condition in criteria]
    ...
    def do_GET(self) -> None:
        ...
        if path == "/health":
            return self.send_json(
                {"status": "ok", "service": "training", "count": len(CATALOG)}
            )
        if path == "/trainings":
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if path == "/start":
                ...
                return self.send_json(
                    {"training": training, "startedAt": int(time.time() * 1000)}
                )
            if path == "/evaluate":
                return self.send_json(
                    evaluate(
...
def main() -> None:
    ...
    print(f"[training] http://{args.host}:{args.port} ({len(CATALOG)} trainings)")
