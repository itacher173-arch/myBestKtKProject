...
Изменение кода и документации выполняется в одном pull request, если меняются публичные команды, переменные среды, API, роли, структура данных или эксплуатационные процедуры. Локальные ссылки проверяются командой `python scripts/check_docs.py` и в CI.
