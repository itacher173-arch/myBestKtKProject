...
UI → gateway/auth → ai-api (orchestrator)
                         ...
                         └─ llama.cpp → Qwen 2.5 1.5B Instruct (GGUF)
```
...
Используется `Qwen2.5-1.5B-Instruct-Q4_K_M.gguf` (около 1 ГБ). Вес хранится
...
Чат работает по гибридной offline-политике. Общие определения могут опираться
на устойчивые знания локальной модели, при этом документация имеет приоритет.
Режимные параметры, аварийные действия и инструкции для реальной установки
формируются только по локальным источникам и контексту симуляции.
Ответы без релевантного локального источника помечаются как требующие проверки:
модель 1.5B не является достоверным техническим справочником.
