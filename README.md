...
Файл `.env.test` включает локальный демонстрационный профиль. Для собственной
конфигурации скопируйте `.env.example` в `.env` и задайте уникальные значения:
...
> [!CAUTION]
> Учётные записи из `.env.test` предназначены только для локальной демонстрации.
> Никогда не используйте их в общей, тестовой или производственной среде и не
> коммитьте собственный `.env`, токены, ключи или реальные данные.
...
| Роль | Логин | Пароль | Куда входит |
| --- | --- | --- | --- |
| Администратор | `admin` | `admin` | <http://localhost:8080/admin/> |
| Инструктор | `instructor` | `instructor` | <http://localhost:8080/> |
| Обучаемый | `trainee` | `trainee` | <http://localhost:8080/> |

Записи создаются автоматически, когда в `.env` установлено
`KTK_DEMO_ACCOUNTS_ENABLED=true`. При каждом старте сервиса авторизации их роли
и пароли восстанавливаются до значений из `.env.test`. Саморегистрация отключена.
