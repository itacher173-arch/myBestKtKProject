...
export function SchemeViewer() {
  ...
  const handleNodeSelect = useCallback(
    (id: string) => {
      if (
        activeMiniTraining &&
        !activeMiniTraining.equipmentIds.includes(id) &&
    ...
    [activeMiniTraining, closePanel, focusZone, selectEquip],
  )
  ...
  return (
    ...
    >
              ...
              return (
                <g key={pipe.id}>
                  <path
                    ...
                    markerEnd={marker}
                    opacity={activeMiniTraining ? 0.2 : 0.9}
                  />
                  {pipe.label && mid && (
                    <text
                      x={mid[0] + 4}
            ...
            {groups.map((node) => (
              <g
                ...
                opacity={
                  !activeMiniTraining ||
                  activeMiniTraining.equipmentIds.includes(node.id) ||
                  activeMiniTraining.zoneIds.includes(node.id)
                    ? 1
                    : 0.12
                }
                pointerEvents={
      ...
      <div className="scheme-hint">
        ...
        {activeMiniTraining
          ? 'Активен сегментный режим: затемнённое оборудование заблокировано'
          : 'Зелёный контур — управление · Клик — выбор · Двойной клик — полное окно · Зоны сверху — переход · Колёсико — масштаб'}
      </div>
